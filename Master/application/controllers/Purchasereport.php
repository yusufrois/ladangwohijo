...
class Purchasereport extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['table_name'] = 'LAPORAN PEMBELIAN :';

		// DEFINES WHICH PAGE TO RENDER
		$data['main_view'] = 'medicine_purchase';
		...
		if ($first_date == NULL OR $second_date == NULL)
		{
			...
			$second_date = date('Y-m-31');
			
			// FETCH SALES RECORD FROM invoices TABLE
			$result_invoices = $this->Accounts_model->fetch_record_purch('mp_temp_record_purc', $first_date, $second_date, $customer_id);
		}
		else
		{
			
			// FETCH SALES RECORD FROM invoices TABLE
			$result_invoices = $this->Accounts_model->fetch_record_purch('mp_temp_record_purc', $first_date, $second_date);
		}
		...
		else
		{
			...
			$data['actionresult'] = "purchasereport/";
			$data['heading1'] = "No purchase available. ";
			$data['heading2'] = "Oops! Sorry no purchase record available in the given details";
