...
class Statements extends CI_Controller
{
	...
	public function income_statement()
	{
		...
		// $data['to'] = $endyear; 
		$startyear = html_escape($this->input->post('from'));
		$endyear   = html_escape($this->input->post('to'));

		if($startyear == NULL OR $endyear == NULL)
		{
	...
	//Vouchers/journal_purchase
	function journal_purchase($no_inv)
	{
		...
		// DEFINES WHICH PAGE TO RENDER
		$data['main_view'] = 'journal_purchase';
	...
	//Statements/create_purcahse
	function create_journal_purchase()
	{
		$description = html_escape($this->input->post('description'));
		$date   = html_escape($this->input->post('date'));	
		$account_head   = html_escape($this->input->post('account_head'));		
		...
		if($status)
		{
			...
			$this->load->model('Transaction_model');
			$sukses = $this->Transaction_model->update_pembelian($account_head[1],$debitamount[1],$description);
			$result = $this->Transaction_model->journal_voucher_entry($data);
	...
	function leadgerPerAccounst()
	{
		...
		$this->load->model('Statement_model');
		$data['ledger_records'] = $this->Statement_model->the_ledger_perAccount($account_head,$year);
		...
		//$data['trail_records'] = $this->Statement_model->trail_balance($year); 
		// DEFINES WHICH PAGE TO RENDER
		$data['main_view'] = 'ledgerperaccount';
