...
var timmer;
             ...
             if(chk && data.length < 16)
             {
                if(data != null)
                {
                    ...
                    timmer = setTimeout(function callback(){
                        ...
                        $.ajax({
                            url: '<?php echo base_url('invoice_2/add_barcode_item/'); ?>'+data,
    ...
    function clear_invoice()
    {
       ...
       // SHOW AJAX RESPONSE ON REQUEST SUCCESS
        $.ajax({
            url: '<?php echo base_url("invoice_2/clear_temp_invoice"); ?>',
            success: function(response)
            {
                jQuery('#inner_invoice_area').html(response);
                 $('#barcode_scan_area').val('');
    ...
    function get_search_result(search_item)
    {
        if(search_item != '')
        {
           // alert('ree');
            // SHOW AJAX RESPONSE ON REQUEST SUCCESS
            $.ajax({
                url: '<?php echo base_url("invoice_2/search_result_manual/"); ?>'+search_item,
                success: function(response)
                {
    ...
    function add_search_item_invoice(id)
    {
        ...
        // SHOW AJAX RESPONSE ON REQUEST SUCCESS
        $.ajax({
            url: '<?php echo base_url('invoice_2/add_selected_item/'); ?>'+id,
            success: function(response)
            {
                jQuery('#inner_invoice_area').html(response);
                 $('#barcode_scan_area').val('');
    ...
    $(document).keyup(function(e) 
    {
       // alert(e.keyCode);
        ...
        { 
            window.location='<?php echo base_url('invoice_2/manage')?>';
        }       
    ...
    function search_customer_payments(cus_id)
    {
        ...
        // SHOW AJAX RESPONSE ON REQUEST SUCCESS
        $.ajax({
            url: '<?php echo base_url('invoice_2/search_previous_cus_balance/'); ?>'+cus_id,
            success: function(response)
            {
    ...
    {       
           clearTimeout(timmer);
            ...
            // SHOW AJAX RESPONSE ON REQUEST SUCCESS
            $.ajax({
                url: '<?php echo base_url('invoice_2/update_qty/'); ?>'+int_val+'/'+item_id,
                success: function(response)
                {
                    jQuery('#inner_invoice_area').html(response);
                     $('#barcode_scan_area').val('');
    ...
    {       
           clearTimeout(timmer);
            ...
            // SHOW AJAX RESPONSE ON REQUEST SUCCESS
            $.ajax({
                url: '<?php echo base_url('invoice_2/update_price/'); ?>'+int_val+'/'+item_id,
                success: function(response)
                {
                    jQuery('#inner_invoice_area').html(response);
                     $('#barcode_scan_area').val('');
