...
class Statement_model extends CI_Model
{
    ...
    public function get_ledger_transactions_year($head_id,$date1)
    {
        $this->db->select("mp_generalentry.id as transaction_id,mp_generalentry.date,mp_generalentry.naration,mp_head.name,mp_head.nature,mp_sub_entry.*");
        $this->db->from('mp_sub_entry');
        $this->db->join('mp_head', "mp_head.id = mp_sub_entry.accounthead");
        $this->db->join('mp_generalentry', 'mp_generalentry.id = mp_sub_entry.parent_id');
        $this->db->where('mp_head.id', $head_id);
        $this->db->where('YEAR(mp_generalentry.date) >=', $date1);
//        $this->db->where('mp_generalentry.date <=', $date2);
        $this->db->order_by('mp_generalentry.date','asc');
        $query = $this->db->get();
        if ($query->num_rows() > 0)
    ...
    public function the_ledger_perAccount($id_head,$date1)
    {
        $accounts_types = array('Assets','Liability','Equity','Revenue','Expense');
        $form_content = '';
        for ($i =0; $i  < count($accounts_types); $i++) 
        { 
            ...
            $this->db->from('mp_head');
            $this->db->where(['mp_head.nature' => $accounts_types[$i]]);
            $this->db->where(['mp_head.id' => $id_head]);
            $query = $this->db->get();
            if ($query->num_rows() > 0)
            {
                $heads_record =  $query->result();
                foreach ($heads_record as $single_head) 
                {
                    if ($this->get_ledger_transactions_year($single_head->id,$date1) != NULL)
                    {
                       $total_ledger = 0;
                       $ledger_query  = array();
                       $form_content .= '<hr />                                       
                        ...
                        <tbody>';
                         $amount1 =  $this->count_head_amount_year($single_head->id,$date1);
                    //var_dump($amount1);
                
                    foreach ($this->get_ledger_transactions_year($single_head->id,$date1) as $single_ledger) 
                    {
                        $debitamount = '';
                        $creditamount = '';
    ...
    public function count_head_amount_year($head_id,$date1)
    {
        $count_total_amt = 0;
        $this->db->select("mp_generalentry.id as transaction_id,mp_generalentry.date,mp_generalentry.naration,mp_sub_entry.*");
        $this->db->from('mp_sub_entry');
    ...
    public function income_statement($date1,$date2)
    {
        ...
        $this->db->or_where(['mp_head.expense_type' => 'pembelian']);
        $query = $this->db->get();
        if($query->num_rows() > 0)
        {
            $record_data =  $query->result();
    ...
    public function balancesheet($end_date)
    {
       ...
       //CURRENT LIBILTY
        ...
        $Pendapatan = $this->Pendapatan($date1,$date2);
        $Pengeluaran = $this->Pengeluaran($date1,$date2);
        $total_libility_equity_retained = $total_equity+$total_cur_libility+$total_noncurrent_libility+$retained_earnings;
        $totalequity_retained = $total_equity+$total_noncurrent_libility+$retained_earnings;
        if(($total_noncurrent+$total_current) == $total_libility_equity_retained)
        {
            $cek = 'Balance ('.number_format(  $total_libility_equity_retained - ($total_noncurrent+$total_current),'0',',','.').')';
        }else{
            $cek = 'Balance sheets are not the same ('.number_format(  $total_libility_equity_retained - ($total_noncurrent+$total_current),'0',',','.').')';
        }
        $equity .= '<tr>
         <td><h4> Laba Ditahan (Pendapatan)</h4></td>
         <td style="text-align:right" ><h4><i>'.number_format($Pendapatan,'0',',','.');'</i></h4></td>
         </tr>';
         $equity .= '<tr>
         <td><h4> Laba Ditahan (Pengeluaran)</h4></td>
         <td style="text-align:right" ><h4><i>'.number_format($Pengeluaran,'0',',','.');'</i></h4></td>
         </tr>';
         $equity .= '<tr>
         <td><h4> Laba Ditahan (Pendapatan - Pengeluaran)</h4></td>
         ...
         <td style="text-align:right;"><h4><i>'.number_format($totalequity_retained,'0',',','.');'</i></h4></td>
         </tr>'; 
         ...
         $total_libility_and_equity .= '<tr class="balancesheet-row">
         <td ><h4><b><i>Check Balance</i></b></h4></td>
         <td style=" text-align:right;" ><h4<b><i>'.$cek;'</i></b></h4></td>
         </tr>';                     
    ...
    public function Pendapatan($start,$end)
    {
        $total_expense = 0;
        $total_revenue = 0;
    ...
    public function Pengeluaran($start,$end)
    {
        $total_expense = 0;
        $total_revenue = 0;
    ...
    function donasi($head_id)
    {
        $count = 0; 
        $this->db->select("sum(jumlah) as jumlah");
        $this->db->from('mp_donasi');
        $this->db->where(['mp_donasi.status' => $head_id]); 
        ...
        if($query->num_rows() > 0)
        {
            ...
            if($result != NULL)
            {
                foreach ($result as $single_head) 
                { 
                    $count = $count + $single_head->jumlah;
                    //$count = $count + $single_head->jumlah;
                }
            }
        }
    ...
    function donasi_now($head_id,$date)
    {
        $count = 0; 
        $this->db->select("*");
        $this->db->from('mp_donasi');
        $this->db->where(['mp_donasi.status' => $head_id, 'DATE(last_update)' => $date]); 
