...
class Crud_model extends CI_Model
{
    ...
    public function tahun_aktif(){
        ...
        $this->db->select('distinct year(date) as tgl');
        $query = $this->db->get('mp_generalentry');
        if ($query->num_rows() > 0)
        {
            return $query->result();
        }
    ...
    public function nama_akun($tablename, $args)
    {
    return $this->db
        ->select('name')
        ->where('id', $args)
    ...
    {
        $this->db->select("mp_expense.*,mp_payee.customer_name, mp_head.name as head_name,mp_head.nature, a.name as nama");
        $this->db->where('mp_expense.date >=', $date1);
        $this->db->where('mp_expense.date <=', $date2);
        $this->db->from('mp_expense');
        $this->db->join('mp_head as a', "mp_expense.method = a.id");
    ...
    public function fetch_record_pupuk($date1,$date2)
    {
        $this->db->select("mp_pupuk.*,mp_head.name, mp_masterpupuk.jenis as jenis_pupuk");
        $this->db->where('jurnal', $date1);
        //$this->db->where('mp_expense.date <=', $date2);
        $this->db->from('mp_pupuk');
        $this->db->join('mp_head', "mp_pupuk.jurnal = mp_head.id");
  //      $this->db->join('mp_head_pupuk', "mp_pupuk.jurnal = mp_head_pupuk.id1");
        $this->db->join('mp_masterpupuk', "mp_pupuk.nama = mp_masterpupuk.nama_pupuk");
        //$this->db->join('mp_payee', "mp_payee.id = mp_expense.payee_id");
        $query = $this->db->get();
        if ($query->num_rows() > 0)
        {
    ...
    public function fetch_report_penjualanmelon($date1,$date2)
    {
        $this->db->select("*");
        $this->db->where('periode', $date1);
        //$this->db->where('mp_expense.date <=', $date2);
        $this->db->from('mp_penjualanmelon');
        $this->db->join('mp_head', "periode = mp_head.id");
  //      $this->db->join('mp_head_pupuk', "mp_pupuk.jurnal = mp_head_pupuk.id1");
        //$this->db->join('mp_masterpupuk', "mp_pupuk.nama = mp_masterpupuk.nama_pupuk");
        //$this->db->join('mp_payee', "mp_payee.id = mp_expense.payee_id");
        $query = $this->db->get();
    ...
    public function fetch_record_hpp($jurnal,$jenis)
    {
        $this->db->select("mp_masterpupuk.jenis,COALESCE(SUM(total),0) as jumlah_all");
  ...
  //      $this->db->join('mp_head_pupuk', "mp_pupuk.jurnal = mp_head_pupuk.id1");
        $this->db->join('mp_masterpupuk', "mp_pupuk.nama = mp_masterpupuk.nama_pupuk");
        //$this->db->group_by('mp_masterpupuk.jenis');
        //$this->db->join('mp_payee', "mp_payee.id = mp_expense.payee_id");
        $query = $this->db->get();
        if ($query->num_rows() > 0)
        {
    ...
    public function fetch_record_benih($jurnal,$jenis)
    {
        $this->db->select("jenis_melon,COALESCE(sum(jumlah),1) as jumlah");
        $this->db->where('periode', $jurnal);
        $this->db->from($jenis);
        $query = $this->db->get();
        if ($query->num_rows() > 0)
        {
    ...
    public function fetch_record_penjualanmelon($jurnal,$jenis)
    {
        $this->db->select("COALESCE(sum(harga),0) as harga");
        $this->db->where('periode', $jurnal);
        $this->db->from($jenis);
        $query = $this->db->get();
        if ($query->num_rows() > 0)
    ...
    public function fetch_record_tonase($jurnal,$jenis)
    {
        $this->db->select("COALESCE(sum(tonase),0) as tonase");
        $this->db->where('periode', $jurnal);
        $this->db->where('grade', $jenis);
        $this->db->from('mp_penjualanmelon');
        $query = $this->db->get();
        if ($query->num_rows() > 0)
        {
    ...
    public function delete_jurnal($tablename)
    {
        $db_debug = $this->db->db_debug;
        $this->db->db_debug = FALSE;
    ...
    public function kurang_retur($id, $val,$tgl){
        ...
        $query_k = $this->db->query("INSERT INTO mp_kadaluarsa (invoice_id,nama_produk, barcode, jumlah, kadaluarsa, tanggal_beli, harga, status_barang) SELECT '0000',product_name,barcode,'".$val."','".$tgl."',CURDATE(), purchase,'0' FROM mp_productslist WHERE id = '".$id."'");
        return true;
    }

    public function gagal_return($id,$jumlah,$user_name){
        $query = $this->db->query("UPDATE mp_productslist_resturn SET jumlah = jumlah - '".$jumlah."' WHERE id_produk = '".$id."'");
        $query_s = $this->db->query("INSERT INTO mp_return_delete (produk_id,jumlah,tanggal,user) VALUES ('".$id."','".$jumlah."',CURRENT_DATE(),'".$user_name."')");
        return true;
