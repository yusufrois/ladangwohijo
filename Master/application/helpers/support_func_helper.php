...
if ( !function_exists('fetch_single_return_productlist'))
{
	//USED TO FETCH AND COUNT THE NUMBER OF OCCURANCE IN RETURN STOCK
	function fetch_single_return_productlist($item_id)
	{
		$CI	=&	get_instance();
		$CI->load->database();
		$CI->db->select("jumlah");
		$CI->db->from('mp_productslist_resturn');
		$CI->db->where(['mp_productslist_resturn.id_produk'=> $item_id]);
		$query = $CI->db->get();
		$result = 0;
		if($query->num_rows()>0)
		{
			...
			if($obj_res != NULL)
			{
				foreach ($obj_res as $single_qty) 
				{
					$result = $result + $single_qty->jumlah;
				}
			} 
		}
